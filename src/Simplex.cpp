...
const uint8_t Simplex::p[256] = { 151, 160, 137, 91, 90, 15, 131, 13, 201, 95,
...
const Grad Simplex::grad3[12] = { Grad(1, 1, 0), Grad(-1, 1, 0), Grad(1, -1, 0),
		Grad(-1, -1, 0), Grad(1, 0, 1), Grad(-1, 0, 1), Grad(1, 0, -1), Grad(-1,
				0, -1), Grad(0, 1, 1), Grad(0, -1, 1), Grad(0, 1, -1), Grad(0,
				-1, -1) };
...
double Simplex::dot(const Grad& g, double x, double y, double z) {
	return g.x * x + g.y * y + g.z * z;
}

// 2D Simplex noise
double Simplex::noise_2d(const double x, const double y) {
...
double Simplex::noise_3d(const double x, const double y, const double z) {
